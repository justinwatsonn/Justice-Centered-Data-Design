...
let lessProfessions = [];
for (let i in profession) {
  if (i != 1) {
    lessProfession.push(profession[i])
  }
}
```
...
for (let prof of professions) {
  console.log("person's profession is " + prof)
}
```
...
```js
let validChildStatus = []

for (let status of childStatus) {
  if (status.length > 0) {
    validChildStatus.push(status)
  }
}
```
...
let sexFullTerm = sex.map(
  (sexItem) => { if (sexItem === "f") return "female";
  else if (sexItem === "m") return "male";
  else if (sexItem === "i") return "intersex";
  else return "unknown";
